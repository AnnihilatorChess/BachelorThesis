...
- `sim.dam_radius`: varies per sample
...
pdebench_swe/
  ...
  data/
    train/
      pdebench_swe_00.hdf5        # split into 8 parts for better I/O performance
      ...
      pdebench_swe_07.hdf5
    valid/
      pdebench_swe_00.hdf5
      ...
      pdebench_swe_07.hdf5
    test/
      pdebench_swe_00.hdf5
...
Each HDF5 file is an independently valid Well-format file containing a subset of the trajectories. This splitting avoids HDF5 global lock contention when using multiple DataLoader workers.
...
### 1. Split Assignment
The 1000 PDEBench samples are split sequentially:
- **Train**: samples 0-799 (800 trajectories)
- **Valid**: samples 800-899 (100 trajectories)
- **Test**: samples 900-999 (100 trajectories)

Verification with `scripts/verify_swe_radii.py` confirms that `dam_radius` is not sorted in the original file, so sequential splitting provides an unbiased distribution of parameters across splits (Train mean: 0.4951, Valid mean: 0.4873, Test mean: 0.5037).
...
# Length: 77,600 (800 trajectories x 97 windows)
# With n_steps_input=1, length is 80,000.
```

The `constant_scalars` tensor contains `dam_radius` for format compatibility with The Well, but is not consumed by any model in this thesis.

## Notes for Training and Performance
...
The `pdebench_swe` dataset contains **800 trajectories** in the training split, which is significantly more than some other Well datasets (e.g., `turbulent_radiative_layer_2D` has only 72). 

- **Samples per Epoch**: ~77,600 samples (at `n_steps_input=4`).
- **Training Time**: On an NVIDIA RTX 1080Ti, training with FNO (`batch_size=32`) takes approximately **15-18 minutes per epoch** (~0.4s per iteration). A full 100-epoch run takes about **25-30 hours**.
- **Comparison**: For context, `turbulent_radiative_layer_2D` has only ~7,000 samples per epoch and trains in ~4 hours for 100 epochs, even though it has higher resolution and more fields. The bottleneck for SWE is simply the high number of trajectories.

### Faster Iteration
If faster training is desired for experimentation, one can use the `include_filters` or `exclude_filters` in the dataset config to only load a subset of the 8 HDF5 files, effectively reducing the number of trajectories.
